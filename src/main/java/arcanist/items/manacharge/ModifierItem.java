...
public class ModifierItem extends Item {
    ...
    public ModifierItem(int stacksize) {
        super(stacksize);
    }
    ...
    public static boolean sameStats(GNDItemMap scribble, GNDItemMap tickle){
        //bop
        for (ModLensModifiers.ModifierEntry entry : ModLensModifiers.flat) {
            if(scribble.getFloat(entry.id) != tickle.getFloat(entry.id)) return false;
        }
        for (ModLensModifiers.ModifierEntry entry : ModLensModifiers.multi) {
            if(scribble.getFloat(entry.id) != tickle.getFloat(entry.id)) return false;
        }
        for (ModLensModifiers.ModifierEntry entry : ModLensModifiers.flags) {
    ...
    public void flat(String id, float value){
        additive.add(new NumberEntry(id, value));
    }
    public void multi(String id, float value){
        multiplicative.add(new NumberEntry(id, value));
    }
    public void flag(String id, boolean value){
